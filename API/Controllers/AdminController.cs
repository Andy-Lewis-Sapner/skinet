    ...
    : BaseApiController
{
    ...
    {
        OrderSpecification spec = new(specParams);
    ...
    {
        OrderSpecification spec = new(id);
        Order? order = await unit.Repository<Order>().GetEntityWithSpec(spec);
        if (order == null) return BadRequest("No order with that it");
        return order.ToDto();
    }
    ...
    public async Task<ActionResult<OrderDto>> RefundOrder(int id)
    {
        ...
        string result = await paymentService.RefundPayment(order.PaymentIntentId);
        if (result == "succeeded")
        {
